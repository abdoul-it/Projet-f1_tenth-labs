...
kp = 1.2
file_waypoint = '/home/nvidia/f1tenth_ws/src/scripts_ZAR/points.csv'
velocity = 3
...
class PurePursuit(object):
    ...
    def __init__(self):
        ...
        self.drive_pub = rospy.Publisher("/vesc/high_level/ackermann_cmd_mux/input/nav_0", AckermannDriveStamped, queue_size=1000)
        waypoints = np.loadtxt(file_waypoint, delimiter=',', usecols=(0, 1))
        self.way_points_list = waypoints
        self.index = -1
    ...
    def way_points_marker(self, way_points_list):
        markers = MarkerArray()
        for i in range(way_points_list.shape[0]):
            marker = Marker()
    ...
    def pose_callback(self, pose_msg):
        ...
        drive_msg.drive.speed = 3 *velocity / (3 + abs(steering_angle))
